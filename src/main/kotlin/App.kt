...
fun saveField(field: Field){
    if(State.saveFile.path != "") {
        ...
        saver.write(listOf(field.size.toString(), field.bornRule.toString(), field.liveMinRule.toString(), field.liveMaxRule.toString(), State.cellSize.toInt().toString()).joinToString(" "))
        saver.newLine()
...
class Field(val path: String = "./data/CommonField.txt", var size: Int = 20, var cells: List<Cell> = listOf(),
            var bornRule: Int = 3, var liveMinRule: Int = 2, var liveMaxRule: Int = 3){
    init{
        try {
            var iterator = 0
            val file = File(path)
            ...
            bornRule = settings[1].toInt()
            liveMinRule = settings[2].toInt()
            liveMaxRule = settings[3].toInt()
            State.cellSize = settings[4].toFloat()
    ...
    fun mouseIn(): Boolean = (State.mouseX - State.offsetX > State.cellSize && State.mouseX - State.offsetX < min(1200-State.offsetX.toInt(), (size+1)*State.cellSize.toInt()) && (State.mouseY - State.offsetY > State.cellSize && State.mouseY - State.offsetY < (size+1)*State.cellSize))
    fun aliveNeighbours(cell: Cell): Int{
        val coordinat = (cell.x-1)*size + (cell.y-1)
        val squareSize = size*size
        return listOf(cells[(coordinat+1 + squareSize)%squareSize], cells[(coordinat-1 + squareSize)%squareSize], cells[(coordinat+1 + size + squareSize)%squareSize],
            cells[(coordinat-1 + size + squareSize)%squareSize], cells[(coordinat+1 - size + squareSize)%squareSize], cells[(coordinat-1 - size + squareSize)%squareSize],
            cells[(coordinat + size + squareSize)%squareSize], cells[(coordinat- size + squareSize)%squareSize]).count{it.isAlive}
    }
    fun nextGeneration(){
        cells = cells.map{
            if(it.isAlive && (this.aliveNeighbours(it) < liveMinRule || this.aliveNeighbours(it) > liveMaxRule)) Cell(it.x, it.y, false, 0)
            else if(!it.isAlive && this.aliveNeighbours(it) == bornRule) Cell(it.x, it.y, true, 1)
    ...
    fun randomField(){
        cells = cells.map{Cell(it.x, it.y, Random.nextBoolean(), 0)}.map{if(it.isAlive) Cell(it.x, it.y, it.isAlive, 1) else it}
    }
}
...
class Renderer(val layer: SkiaLayer): SkiaRenderer {
    ...
    override fun onRender(canvas: Canvas, width: Int, height: Int, nanoTime: Long) {
        ...
        if(State.isClicked) {
            if (State.field.mouseIn()) {
                State.field.transform(State.mouseX - State.offsetX.toInt(), State.mouseY - State.offsetY.toInt())
            }
        ...
        if(State.field.mouseIn()){
            drawLiveTime(canvas, State.mouseX, State.mouseY, State.field.liveTime(State.mouseX - State.offsetX.toInt(), State.mouseY - State.offsetY.toInt()))
        }
        layer.needRedraw()
    }
    private fun drawPlayField(canvas: Canvas, field: Field, offsetX: Float, offsetY: Float, cellSize: Float){
        canvas.drawRect(Rect.makeXYWH(offsetX + cellSize, offsetY + cellSize, cellSize*(field.size), cellSize*(field.size)), deadPaint)
        field.cells.forEach{if(it.x + offsetX/cellSize >= 0 && it.x + offsetX/cellSize < 1200/cellSize.toInt() && it.y + offsetY/cellSize < 2000/cellSize.toInt() && it.y + offsetY/cellSize >= 0 && it.isAlive){
            canvas.drawRect(Rect.makeXYWH(offsetX + cellSize*it.x, offsetY + cellSize*it.y, cellSize, cellSize), alivePaint)}}
    ...
    private fun drawButtons(canvas: Canvas, buttons: List<Button>){
        buttons.forEach {
            canvas.drawRect(Rect.makeXYWH(it.x.toFloat(), it.y.toFloat(), it.w.toFloat(), it.h.toFloat()), Paint().apply { color = 0x55FF4500.toInt() })
...
object SizeActionListener : ActionListener {
    override fun actionPerformed(event: ActionEvent?) {
        val returnVal = JOptionPane.showInputDialog(null, "Size", "Set field size", JOptionPane.YES_NO_OPTION)
        if(returnVal != null){
            if(returnVal.toInt() <= 0)
                JOptionPane.showMessageDialog(null, "Invalid field size", "Error", JOptionPane.ERROR_MESSAGE)
            else
...
object RuleActionListener : ActionListener {
    override fun actionPerformed(event: ActionEvent?) {
        val bornField = JTextField(5)
        val liveMinField = JTextField(5)
        val liveMaxField = JTextField(5)
        val rulePanel = JPanel()
        rulePanel.add(JLabel("Born: "))
        rulePanel.add(bornField)
        rulePanel.add(JLabel("Live min: "))
        rulePanel.add(liveMinField)
        rulePanel.add(JLabel("Live max: "))
        rulePanel.add(liveMaxField)
        val result = JOptionPane.showConfirmDialog(
            null, rulePanel,
            "Set game rules", JOptionPane.OK_CANCEL_OPTION
        )
        if(result == JOptionPane.OK_OPTION){
            if(bornField.text.toInt() > 0 && bornField.text.toInt() <= 8 &&
                liveMinField.text.toInt() > 0 && liveMinField.text.toInt() <= 8 &&
                liveMaxField.text.toInt() >= liveMinField.text.toInt() && liveMaxField.text.toInt() <= 8) {
                State.field.bornRule = bornField.text.toInt()
                State.field.liveMinRule = liveMinField.text.toInt()
                State.field.liveMaxRule = liveMaxField.text.toInt()
            }
            else{
...
object SetNumberOfMovesListener: ActionListener{
    override fun actionPerformed(event: ActionEvent) {
        ...
        val result = JOptionPane.showConfirmDialog(null, numberOfMovesPanel, "Set number of moves", JOptionPane.OK_CANCEL_OPTION)
        if(result == JOptionPane.OK_OPTION) {
            if (isInfinity.isSelected)
                ...
                try {
                    ...
                    JOptionPane.showMessageDialog(null, "Invalid number of moves", "Error", JOptionPane.ERROR_MESSAGE)
                }
            }
        }
    }
